...
import { flashcards, TOPIC } from "@/data/clase1";

export default function FlashcardsPage() {
  const total = flashcards.length;

  // Indices de cards permanentemente dominadas
  const [doneIds, setDoneIds] = useState<Set<number>>(new Set());

  // Cola actual (indices de flashcards a mostrar esta ronda)
  const [queue, setQueue] = useState<number[]>(() => flashcards.map((_, i) => i));

  // Posición dentro de la cola actual
  const [queuePos, setQueuePos] = useState(0);

  // Cards marcadas "a repasar" en esta ronda → formarán la próxima cola
  ...
  // Color de cada dot en la barra de progreso global
        ...
        <div className="w-full max-w-lg">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              {doneIds.size} de {total} dominadas
            </p>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 mb-6">
            ...
            <ul className="flex flex-col gap-2 max-h-52 overflow-y-auto">
              {nextQueue.map((idx) => (
                <li
                  key={idx}
                  className="text-sm text-slate-300 bg-slate-800 rounded-lg px-3 py-2 leading-snug"
                >
          ...
          <div className="flex gap-3">
            ...
            <Link
              href="/"
              className="flex-1 bg-slate-800 hover:bg-slate-700 text-white rounded-xl py-4 transition-colors text-center flex items-center justify-center"
            >
              Salir
            </Link>
          </div>
      ...
      <main className="min-h-screen flex flex-col items-center justify-center px-4 py-16">
        <div className="w-full max-w-lg text-center">
          ...
          <div className="flex gap-3">
            ...
            <Link
              href="/"
              className="flex-1 bg-slate-800 hover:bg-slate-700 text-white font-semibold rounded-xl py-3 transition-colors text-center flex items-center justify-center"
            >
              Inicio
            </Link>
          </div>
  ...
  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-10">
      ...
      <div className="w-full max-w-xl mb-6">
        ...
        {/* Dots: todos los cards del set */}
        <div className="flex gap-1">
          {flashcards.map((_, i) => (
            <div
              key={i}
              className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                !roundDone && i === currentCardIdx
      ...
      <div className="w-full max-w-xl mb-6">
        ...
        >
          <div className={`flip-card-inner ${isFlipped ? "flipped" : ""}`}>
            ...
            {/* Back */}
            <div className="flip-card-back bg-slate-800 border border-slate-600 rounded-2xl flex flex-col justify-center p-8 overflow-y-auto">
      ...
      {isFlipped ? (
        ...
        <div className="w-full max-w-xl">
          <button
            onClick={flip}
            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl py-4 transition-colors"
          >
            Ver respuesta
          </button>
        </div>
